...
class TicketingService:
    ...
    def buy_tickets(self, *, event_id: int, customer_id: int, quantity: int, method: str) -> PurchaseResult:
        ...
        # Transakce: vše nebo nic
        with self.engine.begin() as conn:
            # 1) Zamkni Event řádek (UPDLOCK) a ověř kapacitu + status
            ...
            if ev["Status"] != "Published":
                raise RuntimeError("Lístky lze koupit jen pro Published akce.")
            remaining = int(ev["Capacity"]) - int(ev["SoldCount"])
            if quantity > remaining:
                raise RuntimeError(f"Nedostatečná kapacita. Zbývá {remaining} míst.")

            unit_price = float(ev["TicketPrice"])
            amount = unit_price * quantity

            # 2) Vlož TicketOrder (rovnou Paid – v této fázi nákup = okamžitá platba)
            ...
            # 3) Update SoldCount (constraint CK_Event_Sold_Lte_Capacity hlídá hranici)
            conn.execute(text("""
                UPDATE dbo.Event
            ...
            conn.execute(text("""
                IF NOT EXISTS (
                    SELECT 1 FROM dbo.EventParticipant
                    WHERE EventId = :eid AND CustomerId = :cid
                )
                INSERT INTO dbo.EventParticipant(EventId, CustomerId)
                VALUES (:eid, :cid)
